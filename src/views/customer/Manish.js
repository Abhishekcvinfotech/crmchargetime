...
const Customer = () => {
  ...
  useEffect(() => {
    axios
      .get(`${troesAPi}/usage`)
      .then((res) => setdailyUsusage(res.data / 1000))
      .catch((err) => console.log(err))
  })
  useEffect(() => {
    axios
      .get(`${troesAPi}/sevenusage`)
      .then((res) => {
        setSevenUsuage(res.data)
      })
      .catch((err) => console.log(err))
  }, [])
  ...
  useEffect(() => {
    axios
      .get(`${troesAPi}/monthusage`)
      .then((res) => {
        setMonthUsusage(res.data)
      })
      .catch((err) => console.log(err))
  }, [])
  useEffect(() => {
    axios
      .get(`${troesAPi}/threemonthusage`)
      .then((res) => {
        setThreeMonUsusage(res.data)
      })
      .catch((err) => console.log(err))
  }, [])
  useEffect(() => {
    axios
      .get('https://tro.tentoptoday.com/Development/public/api/yearlyusage')
      .then((res) => {
  ...
  const moreUsusage = sevenUsuage?.reduce((item, val) => +val.daily_uses + item, 0)
  const UsusageMonth = monthUsuage?.reduce((item, val) => +val.daily_uses + item, 0)
  const threeMonths = threeMonUsuage?.reduce((item, val) => +val.daily_uses + item, 0)
  ...
  // const pure_Data = getFullPrice.map((item) => console.log(item, 'item'))
  // console.log(pure_Data, '000')
  ...
  return (
    <>
      <div className="container-fluid customer_information" style={{ position: 'relative' }}>
        ...
        <Modal title="Basic Modal" open={isOpen} onOk={handleOked} onCancel={handleCanceled}>
          <p>Daily Uses : {dailyUsusage}kwh</p>
          <p>Weekly Uses : {moreUsusage.toFixed(3)}kwh</p>
          <p>Monthly Uses : {UsusageMonth}kwh</p>
          <p>Three Month Uses : {threeMonths}kwh</p>
          <p>Yearly Uses : {yEARMonths}kwh</p>
          <p>Remaining Uses: {Math.round(getFullPrice[0] - yEARMonths)}</p>
        </Modal>
